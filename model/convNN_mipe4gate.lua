...
function ModelBuilder:make_net(w2v, p2v)
  ...
  --local ui2h = nn.Linear((#layer1)*opt.num_feat_maps, opt.num_feat_maps)(conv_layer_concat)
  --local uh2h = nn.Linear(opt.num_feat_maps, opt.num_feat_maps)(dconv_layer_concat)

  --local update_gate = nn.Sigmoid()(nn.CAddTable()({ui2h, uh2h})) -- compute z

  --local ri2h = nn.Linear((#layer1)*opt.num_feat_maps, opt.num_feat_maps)(conv_layer_concat)
  --local rh2h = nn.Linear(opt.num_feat_maps, opt.num_feat_maps)(dconv_layer_concat)  
  --local reset_gate = nn.Sigmoid()(nn.CAddTable()({ri2h, rh2h})) -- compute r

  --local gated_hidden = nn.CMulTable()({reset_gate, dconv_layer_concat}) -- element wise mult z and rrep
  --local p2 = nn.Linear(opt.num_feat_maps, opt.num_feat_maps)(gated_hidden)
  --local p1 = nn.Linear((#layer1)*opt.num_feat_maps, opt.num_feat_maps)(conv_layer_concat)
  ---- compute hidden candidate
  --local hidden_candidate = nn.Tanh()(nn.CAddTable()({p1,p2}))
  ----local hidden_candidate = nn.Linear((#layer1)*opt.num_feat_maps, opt.num_feat_maps)(conv_layer_concat)
        
  --local zh = nn.CMulTable()({update_gate, hidden_candidate})
  --local zhm1 = nn.CMulTable()({nn.AddConstant(1,false)(nn.MulConstant(-1,false)(update_gate)), dconv_layer_concat})
  --local last_layer = nn.CAddTable()({zh, zhm1})
