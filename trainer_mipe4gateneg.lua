...
function Trainer:train(train_data, train_e1, train_e2, train_dep, train_dep_e1, train_dep_e2, train_labels, mi_labels, model, criterion, micriterion, optim_method, layers, state, params, grads, singletrain, singletrain_e1, singletrain_e2, singletrain_dep, singletrain_dep_e1, singletrain_dep_e2, singletrain_label, singlemitrain_label)
  model:training()

  local train_size = train_data:size(1)
  local single_train_size = singletrain:size(1)
  ...
  local singlenum_batches = math.floor(single_train_size / opt.batch_size)
  local singleshuffle = torch.randperm(singlenum_batches)
  for j = 1, singleshuffle:size(1) do
    local single_t = (singleshuffle[j] - 1) * opt.batch_size + 1
    local single_batch_size = math.min(opt.batch_size, single_train_size - single_t + 1)

    local single_inputs = singletrain:narrow(1, single_t, single_batch_size) --narrow(dim,index, size)
    local single_input_p1s = singletrain_e1:narrow(1, single_t, single_batch_size)
    local single_input_p2s = singletrain_e2:narrow(1, single_t, single_batch_size)
    local single_indep = singletrain_dep:narrow(1, single_t, single_batch_size)
    local single_indep_p1s = singletrain_dep_e1:narrow(1, single_t, single_batch_size)
    local single_indep_p2s = singletrain_dep_e2:narrow(1, single_t, single_batch_size)
    local single_targets = singletrain_label:narrow(1, single_t, single_batch_size)
    local single_mitargets = singlemitrain_label:narrow(1, single_t, single_batch_size)
    
    single_inputs = single_inputs:double()
    single_input_p1s = single_input_p1s:double()
    single_input_p2s = single_input_p2s:double()
    single_indep = single_indep:double()
    single_indep_p1s = single_indep_p1s:double()
    single_indep_p2s = single_indep_p2s:double()
    single_targets = single_targets:double()
    single_mitargets = single_mitargets:double()
    ...
    local func3 = function(x)
      ...
      grads:zero()
      -- forward pass
      local single_output = model:forward({single_inputs, single_input_p1s, single_input_p2s, single_indep, single_indep_p1s, single_indep_p2s, single_inputs, single_input_p2s, single_input_p1s, single_indep, single_indep_p2s, single_indep_p1s})
      local single_outputs, single_mioutputs, single_sm_outputs, single_sm_mioutputs = unpack(single_output)

      local single_err = criterion:forward(single_outputs, single_targets)
      local single_mierr = micriterion:forward(single_mioutputs, single_mitargets)
      -- track errors and confusion
      -- track errors and confusion
      total_err = total_err + single_err * single_batch_size
      total_err = total_err + single_mierr * single_batch_size

      for j = 1, single_batch_size do
        confusion:add(single_outputs[j], single_targets[j])
      end

      -- compute gradients
      local single_sm_df_do = torch.zeros(single_batch_size, opt.num_classes)
      local single_sm_midf_do = torch.zeros(single_batch_size, opt.num_classes)

      -- compute gradients
      local single_df_do = criterion:backward(single_outputs, single_targets)
      local single_midf_do = micriterion:backward(single_mioutputs, single_mitargets)
      single_midf_do:zero()
      model:backward({single_inputs, single_input_p1s, single_input_p2s, single_indep, single_indep_p1s, single_indep_p2s, single_inputs, single_input_p2s, single_input_p1s, single_indep, single_indep_p2s, single_indep_p1s}, {single_df_do, single_midf_do, single_sm_df_do, single_sm_midf_do})
    ...
    -- gradient descent
    optim_method(func3, params, config, state)
    -- reset padding embedding to zero
    -- print(layers.w2v.weight:size())
    -- print(layers.p2v.weight:size())
    layers.w2v.weight[1]:zero()
